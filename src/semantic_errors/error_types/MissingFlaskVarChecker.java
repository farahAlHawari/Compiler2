...
public class MissingFlaskVarChecker {
    ...
    /**
     * ✅ يتحقق إذا كان المتغير معرّف محلياً في Jinja عبر:
     * - {% set %} أو {% with %}  → type="jinja_set_var"
     * - {% for x in y %}         → type="jinja_iterator"
     * (بدل ما نعتبره missing من Flask)
     *
     * Round 11: Added jinja_iterator — loop variables like `p` in
     * {% for p in products %} are locally defined by the for loop
     * itself and should NOT be flagged as missing from Flask.
     */
    private boolean isLocallyDefinedInJinja(String varName) {
        for (SymbolEntry entry : symbolTable.getAllEntries()) {
            if (!"template".equals(entry.getSource())) continue;
            if (varName.equals(entry.getName())) {
                String type = entry.getType();
                // Locally defined via {% set x = ... %}
                if ("jinja_set_var".equals(type)) return true;
                // Locally defined via {% for x in y %} — the loop iterator
